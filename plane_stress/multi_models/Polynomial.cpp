...
Vector2d Polynomial::compute_derivative(Vector2d& lambda){
    ...
    dW_dI[0]= c[0]+2.0*c[2]*(invar[0]-3)+3.0*c[5]*sq(invar[0]-3)+2.0*c[6]*(invar[0]-3)*(invar[1]-3);
    dW_dI[1]= c[6]*sq(invar[0]-3);
...
Matrix2d& Polynomial::compute_stiff_principal(Vector2d& lambda){
    ...
    d2AI1_dAdA = 1.0+1.0/sq(lambda(0))/lambda(1);
    d2AI1_dAdB = 1.0/sq(lambda(1))/lambda(0);

    d2BI1_dBdB = 1.0+1.0/sq(lambda(1))/lambda(0);
    d2BI1_dBdA = 1.0/sq(lambda(0))/lambda(1);

    d2AI2_dAdA = lambda(1)+1.0/sq(lambda(0));
    d2AI2_dAdB = lambda(0);

    d2BI2_dBdB = lambda(0)+1.0/sq(lambda(1));
    d2BI2_dBdA = lambda(1);
