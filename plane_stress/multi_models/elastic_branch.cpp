...
elastic_branch::elastic_branch(){
mat_tan.setZero();
}

void elastic_branch::compute_be_tr(Matrix2d F){
  be_tr = F*(F.transpose());
...
void elastic_branch::compute_stress_tau_principal(Vector2d dW_d){
  tau_principal(0) = 2.0*dW_d(0)*eigs_tr(0);
  tau_principal(1) = 2.0*dW_d(1)*eigs_tr(1);
  p = -(tau_principal(0)+tau_principal(1))/3.0;
};

Matrix2d elastic_branch::compute_stress_tau(){
  tau = tau_principal(0)*v0_*v0_.transpose()+tau_principal(1)*v1_*v1_.transpose();
  return tau;
};


void elastic_branch::compute_tangent_pressure(){
...
void elastic_branch::mat_tan_principal(){
    C_alg =  Stiff_principal;
...
Matrix3d elastic_branch::rotate_mat_tan(){
  mat_tan_principal();
